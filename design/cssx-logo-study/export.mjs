...
for (const v of variants) {
  const points = Array.from({ length: 401 }, (_, i) => {
    ...
    return [
      (v.radial ? radius * Math.cos(t) : 7 * Math.cos(t) - v.d * Math.cos(v.n * t)) * scale,
  ...
  writeFileSync(
    `${destination}${v.id}.svg`,
    `<svg xmlns="http://www.w3.org/2000/svg" viewBox="-64 -64 128 128" role="img"><title>${v.name}</title>${mark}</svg>\n`,
  );
  if (v.n !== 7)
    writeFileSync(
      `${destination}${v.id}-lockup.svg`,
      `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 420 144" role="img"><title>${v.name} lockup</title><g transform="translate(72 72)">${mark}</g><g transform="translate(164 38)">${wordmark}</g></svg>\n`,
    );
}
